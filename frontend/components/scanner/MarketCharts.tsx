  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-8">
      ...
      <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-4 md:p-6 flex flex-col">
        <h3 className="text-xs md:text-sm font-bold text-zinc-400 uppercase tracking-wider mb-4 flex items-center gap-2">
      ...
      <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* Active Events Card */}
        <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-4 md:p-6 flex flex-col justify-between relative overflow-hidden min-h-[160px]">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
          <div>
            <div className="flex items-center gap-2 text-blue-400 mb-2">
              <Layers className="w-4 h-4 md:w-5 md:h-5" />
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Active Events</span>
            </div>
            <div className="text-3xl md:text-4xl font-mono font-bold text-white">{stats.activeEvents}</div>
            <div className="text-zinc-500 text-[10px] md:text-xs mt-1">Events currently live on-chain</div>
          </div>

          {/* Mini Bar Chart of categories */}
          <div className="mt-4 md:mt-6 flex items-end gap-1 h-8 md:h-12 opacity-50">
            {chartData.map((cat, i) => (
              <div
                key={i}
        ...
        <div className="bg-zinc-900/30 border border-white/5 rounded-xl p-4 md:p-6 flex flex-col justify-between relative overflow-hidden min-h-[160px]">
          ...
          <div>
            <div className="flex items-center gap-2 text-emerald-400 mb-2">
              ...
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Top Category</span>
            </div>
            <div className="text-2xl md:text-3xl font-bold text-white truncate">
              {stats.topCategory?.name || 'N/A'}
            </div>
            <div className="text-zinc-500 text-[10px] md:text-xs mt-1 font-mono">
        ...
        {/* Expiring Soon Card */}
        <div className="sm:col-span-2 bg-zinc-900/30 border border-white/5 rounded-xl p-4 md:p-5 flex items-center justify-between relative overflow-hidden group">
          <div className="absolute inset-0 bg-amber-500/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
          <div className="flex items-center gap-3 md:gap-4 relative z-10">
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20">
              <AlertCircle className="w-5 h-5 md:w-6 md:h-6 text-amber-500" />
            </div>
            <div>
              <div className="text-white font-bold text-xs md:text-sm">Approaching Expiry</div>
              <div className="text-zinc-500 text-[10px] md:text-xs">Events resolving within 24 hours</div>
            </div>
          </div>
          <div className="text-right relative z-10">
            <div className="text-2xl md:text-3xl font-mono font-bold text-white">{stats.expiringSoon}</div>
